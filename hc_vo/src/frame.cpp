...
namespace hc_vo
{
	...
	void Frame::createImgPyramid(const cv::Mat& img_level_0, int n_levels, ImgPyr& pyr)
	{
        ...
        //下采样函数
        void Frame::halfSample(cv::Mat& img_l, cv::Mat& img_h)
        {
        }
	/// 帧处理的实用函数
	namespace frame_utils {
		...
		bool getSceneDepth(const Frame& frame, double& depth_mean, double& depth_min)
		{
			...
			return true;
                ...
                {      
                       double sum=0.0,depth_mean;
                       int i=0;
                       std::vector<double>::iterator it_begin=depth_vec.begin();
                       std::vector<double>::iterator it_end=depth_vec.end();
                       for(std::vector<double>::iterator it=it_begin;it!=it_end;++it)
                       {
                          i++;
                          sum+=*it;
                       }
                       return depth_mean=sum/i;
                }
